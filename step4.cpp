...
int main() {
    ...
    int32_t* longtitudes = (int32_t*)malloc(pointNum * sizeof(int32_t));
    int32_t* latitudes = (int32_t*)malloc(pointNum * sizeof(int32_t));

    // 2. ПРОВЕРКА ОБЕИХ ОБЛАСТЕЙ ПАМЯТИ (Защита от сбоя)
    if (altitudes == NULL || longtitudes == NULL || latitudes == NULL) {
        ...
        if (latitudes != NULL) free(latitudes);
        return -1;
    }
    ...
    for (int i = 0; i < pointNum; i++) {
        ...
        cout << "Точка #" << (i + 1) << latitudes[i] << endl;
    }
